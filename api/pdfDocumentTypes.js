...
module.exports = [
  {
    ...
    claudePrompt: `以下はWard VillageのAppointment Request Form（アポイントメント申請フォーム）のテキストです。
...
JSONのみ返してください。説明文・コードブロック記号は不要です。`,
...

  {
    ...
    claudePrompt: `以下はMahana Ward VillageのResidence Preference Form（居住希望フォーム）のテキストです。
フォームから以下の項目をJSON形式で抽出してください。見つからない場合はnullとしてください。
